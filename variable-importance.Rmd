...
#X_raw2 = subset(X_raw, select=c(scau23, scau25, vltveg, updrs_score_p3, drmagrac, np2eat))

# summary(X_raw$np2eat)
# table(X_raw$np2eat)
...
vim = try(
  ...
  #varImpact(Y, X, V=V, g.library=g_lib, Q.library=Q_lib,
            parallel = F, minCell=minCell, family = "gaussian",
...
if (class(vim) == "try-error") {
  ...
  # Make a copy to customize the rounding.
  #vim_latex = vim
  #vim_latex$results_consistent$Estimate = round(vim_latex$results_consistent$Estimate, 2)
