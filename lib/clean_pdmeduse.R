...
clean_pdmeduse = function(df) {
  
  ################################
  # Manual review.
  if (F) {
    df = files$use_of_pd_medication
    dim(df)
    names(df)
    str(df)
  ...
  df = subset(df, select = c(patno, pdmedyn, onldopa, ondopag,
                             onother, fulnupdr, pdmeddt, pdmedtm,
                             nupdrtm))
  
  df$fulnupdr = as.numeric(df$fulnupdr)
  df$pdmeddt = as.numeric(as.Date(paste("01/",df$pdmeddt,sep=""), format = "%d/%m/%Y"))
  df$pdmedtm = as.numeric(as.POSIXct(df$pdmedtm, format="%H:%M:%S"))
  df$nupdrtm = as.numeric(as.POSIXct(df$nupdrtm, format = "%H:%M:%S"))
